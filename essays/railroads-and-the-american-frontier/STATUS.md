# STATUS — Railroads & the American Frontier

*Per-essay tracker. Read this first on session resume. Update at the end of each work cluster.*

**Series:** Part 1 of "The Beginning of the Beginning" (3-part)
**Word target:** ~3,250 of prose (~3,000 series target + lead/thesis)
**Last commit:** `ff31fe6` — draft: Section 1 prose (Technical Unlock)

---

## Phases

| # | Phase | Status | Detail |
|---|---|---|---|
| 1 | Brainstorm | ✅ done | thesis ("optionality engine") + 7-section outline scaffolded |
| 2 | Research | 🔬 ~85% | 18 of ~22 source clusters cited; 4 follow-ups open |
| 3 | Charts | 📊 ~60% | 3 of 5 done (mileage, land grants, railroad capital) |
| 4 | Draft | ✍️ ~49% | lead + thesis + §§ 1-3 in prose (3 of 7 sections) |
| 5 | Polish | — | not started |
| 6 | Publish | — | not started |

**Current focus:** Drafting Section 4 (Who Built It); chart for § 6 (Fogel) and § 5 (second-order timeline) still ahead.

---

## Phase 2 — Research

Verified and cited in `data/sources.md`:

- [x] Railroad mileage by decade, 1830-1916
- [x] Pacific Railroad Acts (1862, 1864) — land grant + bond rates
- [x] Federal land grants total (~131M federal / ~180M with state)
- [x] Homestead Act (~270M acres, ~1.6M claims, 1862-1934)
- [x] NYSE railroad share (>50% capitalization, >80% trading volume)
- [x] Total railroad capitalization ($4.6B 1876 → $10.6B 1890)
- [x] Crédit Mobilier scandal ($50M actual / $94M billed / $44M skim)
- [x] Panic of 1873 / Jay Cooke / Northern Pacific
- [x] Chinese labor on Central Pacific (10-15K peak, ~90%, 1,000+ deaths, 1867 strike)
- [x] Fogel social savings (2.7% agriculture / 6-7% broader)
- [x] Donaldson & Hornbeck modern reanalysis (3.22% of GDP)
- [x] Standard time / "Day of Two Noons" (Nov 18, 1883)
- [x] Union Stock Yards / Chicago meatpacking
- [x] Sears + Montgomery Ward founding + Rural Free Delivery
- [x] Standard Oil railroad rebates (Lake Shore 1868, South Improvement 1871)
- [x] Telegraph along railroad rights-of-way + Western Union dominance
- [x] Civil War USMRR / Sherman's Atlanta campaign supply
- [x] Swift refrigerated cars (1878-1880)

Open follow-ups:

- [ ] Yellowstone / Yosemite / Grand Canyon — railroad-marketing primary sources (Northern Pacific, Santa Fe)
- [ ] Robber baron wealth specifics (Vanderbilt, Stanford, Huntington, Harriman, Gould — peak fortunes vs GDP)
- [ ] Sherman Antitrust Act (1890) + Interstate Commerce Act (1887) primary text references
- [ ] Homestead Act primary source verification for 270M / 1.6M figures (have Wikipedia + National Archives, want one more)

---

## Phase 3 — Charts

- [x] `us-railroad-mileage-1830-1916.png` — § 1 buildout curve
- [x] `federal-land-comparison-19th-century.png` — § 2 land grant comparison
- [x] `railroad-capital-vs-federal-debt-1890.png` — § 3 finance (capitalization growth + scale vs federal debt)
- [ ] `fogel-social-savings.png` — § 6 contrarian (Fogel 2.7% vs Donaldson-Hornbeck 3.22%, with 3-month-delay counterfactual)
- [ ] `second-order-effects-timeline.png` — § 5 (time zones, Chicago stockyards, Sears, Standard Oil, refrigerated cars on a single timeline)
- [ ] *(optional)* `chinese-labor-breakdown.png` — § 4 if data supports a clean visual

---

## Phase 4 — Draft

Sections (target words / current prose):

- [x] Lead paragraph (~100 / done)
- [x] Thesis statement (~150 / done)
- [x] § 1 The Technical Unlock (~350 / ~370)
- [x] § 2 The Government's Bet (~500 / ~415)
- [x] § 3 The Money (~550 / ~560)
- [ ] § 4 Who Built It (~350 / 0)
- [ ] § 5 Second-Order Effects (~600 / 0)
- [ ] § 6 The Contrarian View / Fogel (~400 / 0)
- [ ] § 7 How Big, Really (~250 / 0)

**Current prose:** ~1,595 words (lead + thesis + §§ 1-3).

Outline corrections still pending in DRAFT.md:

- [x] Land grants: 175M → 131M federal / 180M total *(done in commit `86a9338`)*
- [x] § 3 NYSE figure: "60% of NYSE by 1900" → ">50% of capitalization, >80% of trading volume from Civil War to ~1900" *(done in this commit cluster)*
- [ ] § 6 Fogel: "social savings ~5% of GDP?" → "2.7% agriculture / 6-7% broader; Donaldson & Hornbeck 2013 re-estimate 3.22% via land-value channel"

---

## Phase 5 — Polish

- [ ] Strip `## Research Notes` from FINAL.md
- [ ] Copy edit pass for voice (direct, no em dashes, no AI tells)
- [ ] Verify every numerical claim in prose ties to a `data/sources.md` entry
- [ ] Confirm every chart is referenced in the draft and renders correctly

---

## Phase 6 — Publish

- [ ] Substack draft posted
- [ ] Chart images uploaded to Substack
- [ ] `meta.json` updated: `status: published`, `published` date, `substackUrl`
- [ ] `README.md` essay table updated (status icon → ✅ Final)
- [ ] Tweet drafted

---

## Notes

- Series target: 3,000 words. Current outline is heavier in §§ 2, 3, 5 and lighter in §§ 1, 4, 7. Word distribution above reflects that.
- Section 7 should set up Part 2 (Space Industry 2026) without summarizing it.
